...
class PDP_learning:
    ...
    def compute_loss_ioc(self, quad, pop_quadcopter, lqr_traj_solver, trajectories, true_weights):
        ...
        for epoch in range(int(1e4)): # iteration loop (or epoch loop)
            ...
            for iter in range(n_tryout):
                ...
                traj = pop_quadcopter.popSolver(tryout_initial_state, tryout_horizon, current_weights)
                # Auxiliary control system
                aux_sys = pop_quadcopter.getAuxSys(state_trajectories=traj['state_trajectories'],
                ...
                aux_sol = lqr_traj_solver.lqrSolver(np.zeros((pop_quadcopter.n_state, pop_quadcopter.n_weights)), tryout_horizon)
        ...
        sio.savemat('./data_pdp_quadcopter/PDP_results_trial_' + str(trial) + '.mat', {'results': save_data})
