...
class PDP_learning:
    ...
    def compute_loss_ioc(self, pend, pop_pendulum, lqr_solver, trajectories, true_weights):
        ...
        for epoch in range(int(1e4)): # iteration loop (or epoch loop)
            ...
            for iter in range(n_tryout):
                ...
                traj = pop_pendulum.popSolver(tryout_initial_state, tryout_horizon, current_weights)
                # Auxiliary control system
                aux_sys = pop_pendulum.getAuxSys(state_trajectories=traj['state_trajectories'],
                ...
                lqr_solver.initialize_stage_cost(Hxx=aux_sys['Hxx'], Huu=aux_sys['Huu'], Hxu=aux_sys['Hxu'], Hux=aux_sys['Hux'],
                                       Hxe=aux_sys['Hxe'], Hue=aux_sys['Hue'])
                lqr_solver.initialize_terminal_cost(hxx=aux_sys['hxx'], hxe=aux_sys['hxe'])
                aux_sol = lqr_solver.lqrSolver(np.zeros((pop_pendulum.n_state, pop_pendulum.n_weights)), tryout_horizon)
        ...
        sio.savemat('./data_pdp_pendulum/PDP_results_trial_' + str(trial) + '.mat', {'results': save_data})
